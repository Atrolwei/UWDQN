...
class Elephant(Agent):
    def __init__(self,field_size,goal,obs_range=5,style='moderate'):
    ...
    def _potential_field(self,obs):
        ...
        ele_pos=obs[0]
        delta_goal=np.array(self.goal)-np.array(ele_pos)    # intrinsive force of the goal
        ...
        for preyer_pos in obs[1:]:
            if distance.euclidean(ele_pos,preyer_pos)<=obs_range:
                ...
                delta_force-=self.K_go*(1/delta_preyer_norm-1/obs_range)*delta_preyer/delta_preyer_norm**3
        # if N_preyer_obs>0:
    ...
    def _get_action_by_force(self,force):
        ...
        if force_norm<0.02:
            ...
            n_mod=round(mod/45)     # 确定靠近哪个45°方向
            n=n+n_mod       # 确定转多少个45°
            if n==0 or n==8:
                action=2
            elif n==1:
